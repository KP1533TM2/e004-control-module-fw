...
static const bool TABLE_CRC = 1; //включение табличного способа расчета CRC
...
TWake::TWake(const uint8_t frame)
{
  Frame = frame;
  RxData = new uint8_t[frame + PTR_DAT + 1]; //буфер приема
  TxData = new uint8_t[frame + PTR_DAT + 1]; //буфер передачи
  Addr = 0;
  TxState = WST_DONE;
  RxState = WST_IDLE;
}
...
void TWake::Do_Crc8(uint8_t b, uint8_t *crc)
{
  ...
  {
    for(uint8_t i = 0; i < 8; b = b >> 1, i++) //вычисление в цикле
